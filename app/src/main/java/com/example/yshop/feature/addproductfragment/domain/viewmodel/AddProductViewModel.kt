package com.example.yshop.feature.addproductfragment.domain.viewmodel
        ...
        return when {
            ...
            TextUtils.isEmpty(etProductTitle.value.toString().trim { it <= ' ' }) -> {
                OptionBuilder.showErrorSnackBar(
                    context.getString(R.string.err_msg_enter_product_title),
                    true,
                    context,
                    view
                )
            ...
            TextUtils.isEmpty(etProductPrice.value.toString().trim { it <= ' ' }) -> {
                OptionBuilder.showErrorSnackBar(
                    context.getString(R.string.err_msg_enter_product_price),
                    true,
                    context,
                    view
                )
            ...
            TextUtils.isEmpty(etProductDescription.value.toString().trim { it <= ' ' }) -> {
                OptionBuilder.showErrorSnackBar(
                    context.getString(R.string.err_msg_enter_product_description),
                    true,
                    context,
                    view
                )
            ...
            TextUtils.isEmpty(etProductQuantity.value.toString().trim { it <= ' ' }) -> {
                OptionBuilder.showErrorSnackBar(
                    context.getString(R.string.err_msg_enter_product_quantity),
                    true,
                    context,
                    view
                )
    ...
    fun addProductDetails(context: Context, view: View, imageUri: Uri) {
        ...
        if (validateInput(context, view)) {
            ...
            var refStorage: StorageReference =
                productStorage.child("ProductPhoto/" + Constants.PRODUCT_IMAGE_FILE + System.currentTimeMillis())
                ...
                if (saveImage.isSuccessful) {
                    ...
                    refStorage.downloadUrl.addOnSuccessListener { imageUri ->
                        ...
                        viewModelScope.launch {
                            var firstName =
                                DataStoreRepository(context).showFirstName(Constants.FIRST_NAME_KEY)
                            var lastName =
                                DataStoreRepository(context).showLastName(Constants.LAST_NAME_KEY)
                            ...
                            Toast.makeText(
                                context,
                                context.getString(R.string.product_uploaded_success_message),
                                Toast.LENGTH_SHORT
                            ).show()
                        }
                    }
                    ...
                    OptionBuilder.showErrorSnackBar(
                        "Error for uploaded product image ",
                        true,
                        context,
                        view
                    )
