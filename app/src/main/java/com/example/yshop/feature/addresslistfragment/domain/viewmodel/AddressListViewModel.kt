package com.example.yshop.feature.addresslistfragment.domain.viewmodel
...
import com.example.yshop.feature.adapter.RecyclerAddressAdapter
...
class AddressListViewModel : ViewModel() {
    ...
    fun getAddressList(
        ...
        addressReference.orderByChild(Constants.USER_ID).equalTo(Constants.getCurrentUser())
                        ...
                        if (fragmentAddressList.arguments?.containsKey(Constants.EXTRA_ADDRESS_DETAILS) != true) {
                            ...
                            val deleteSwipeHandler = object : SwipeToDeleteCallback(context) {
                                override fun onSwiped(
                                    ...
                                    addressReference.child(addressArray[viewHolder.adapterPosition].id)
                                        .removeValue()
                                    OptionBuilder.hideProgressDialog()
                                    Toast.makeText(
                                        context,
                                        context.resources.getString(R.string.err_your_address_deleted_successfully),
                                        Toast.LENGTH_SHORT
                                    ).show()
                                }
                            }
